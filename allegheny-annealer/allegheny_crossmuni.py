"""
Allegheny County, PA 2024 Presidential Race — Cross-Municipal Correlation Script
==================================================================================
Same question as the municipal-bounded script, but with no municipal constraints.
Precincts may merge with ANY contiguous neighbour regardless of municipality.

This answers: "What if precinct boundaries had been drawn differently county-wide,
without regard to city limits?" — i.e. the annexation/consolidation question.

Rules:
  - Any two spatially contiguous precincts may be merged into one reporting unit.
  - All groups must remain spatially contiguous at all times.
  - Total number of groups is bounded: MIN_GROUPS ≤ n_groups ≤ MAX_GROUPS.
  - HarrisPct is always recalculated from raw vote totals.

State representation:
  - A flat integer numpy array `label` of length N_PRECINCTS.
  - label[i] = group id for precinct i.
  - Groups are identified by their lowest-member precinct index (canonical label).
  - _fast_sr2 uses np.bincount for vectorised group summation.
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "crossmuni")
...
PICKLE_POS     = r"os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "crossmuni")\best_pos_label.npy"
PICKLE_NEG     = r"os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "crossmuni")\best_neg_label.npy"
...
def plot_scenario_label(label: np.ndarray, title: str,
    ...
    ax.set_xlabel("EDay Total Votes in Group", fontsize=10)
